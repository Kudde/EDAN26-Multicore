...
class spinlock_t {
	std::atomic<bool> flagerino;
public:
	void lock() {
		bool expected = false;
		while(!flagerino.compare_exchange_weak(expected, true, std::memory_order_acquire, std::memory_order_relaxed)) {
			expected = false;
		}
	}

	void unlock() {
		flagerino.store(false, std::memory_order_release);
	}
